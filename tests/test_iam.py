...
@terraform('iam_access_key_functional_good', teardown=terraform.TEARDOWN_IGNORE)
def test_access_key_functional_good_policy(test, iam_access_key_functional_good):
    """
    ...
    This test uses terraform to create IAM users and access keys, then validates
    that the iam-access-key resource correctly identifies and filters
    access keys based on various criteria.
    """
    session_factory = test.replay_flight_data("test_iam_access_key_functional_good")

    # Get terraform outputs
    test_user_1 = iam_access_key_functional_good["test_user_1_name"]
    test_user_2 = iam_access_key_functional_good["test_user_2_name"]
    key_1_id = iam_access_key_functional_good["user_1_key_1_id"]
    key_2_id = iam_access_key_functional_good["user_2_key_1_id"]
    key_3_id = iam_access_key_functional_good["user_1_key_2_id"]
    ...
    assert found_key_ids == expected_key_ids

    # Verify required fields are present
    for resource in resources:
        assert 'AccessKeyId' in resource
        assert 'UserName' in resource
        assert 'Status' in resource
        assert 'CreateDate' in resource
    ...
    for resource in resources:
        ...
        assert resource['AccessKeyId'] in {key_1_id, key_2_id}
    ...
    assert len(resources) == 2
    for resource in resources:
        assert resource['UserName'] == test_user_1
        assert resource['AccessKeyId'] in {key_1_id, key_3_id}
    ...
    assert len(resources) == 3
    for resource in resources:
        assert resource['UserName'] in [test_user_1, test_user_2]


@terraform('iam_access_key_functional_bad', teardown=terraform.TEARDOWN_IGNORE)
def test_access_key_functional_bad_policy(test, iam_access_key_functional_bad):
    """
    ...
    boundary conditions.
    """
    session_factory = test.replay_flight_data("test_iam_access_key_functional_bad")

    # Get terraform outputs
    test_user_empty = iam_access_key_functional_bad["test_user_empty_name"]
