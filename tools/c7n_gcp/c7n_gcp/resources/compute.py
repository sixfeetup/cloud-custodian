...
@RegionCommitment.action_registry.register('insert')
class RegionCommitmentInsert(MethodAction):
    """Create a regional commitment.

    This action executes a single `regionCommitments.insert` call using values
    provided in action configuration.
    """
    ...
    schema = type_schema(
        ...
        plan={'enum': ['TWELVE_MONTH', 'THIRTY_SIX_MONTH']},
        type={'enum': ['GENERAL_PURPOSE', 'ACCELERATOR_OPTIMIZED', 'MEMORY_OPTIMIZED',
                       'LOCAL_SSD', 'GENERAL_PURPOSE_N1', 'GENERAL_PURPOSE_N2',
                       'GENERAL_PURPOSE_N2D', 'GENERAL_PURPOSE_E2', 'GENERAL_PURPOSE_T2D']},
        category={'enum': ['MACHINE', 'LICENSE']},
        ...
        license_resource={
            'type': 'object',
            'required': ['amount', 'cores-per-license', 'license'],
            'properties': {
                'amount': {'type': 'string'},
                'cores-per-license': {'type': 'string'},
                'license': {'type': 'string'}
            },
    ...
    method_spec = {'op': 'insert'}
    permissions = ('compute.commitments.create',)

    @staticmethod
    def get_insert_body(data):
        body = {
            'name': data['name'],
            'plan': data['plan'],
        }
        ...
        if 'auto_renew' in data:
            body['autoRenew'] = data['auto_renew']
        if 'license_resource' in data:
            license_resource = data['license_resource']
            body['licenseResource'] = {
                'amount': license_resource['amount'],
                'coresPerLicense': license_resource['cores-per-license'],
                'license': license_resource['license']
            }
        return body
    ...
    def process(self, resources):
        ...
        project = self.data.get('project', session.get_default_project())
        params = {
            'project': project,
            'region': self.data['region'],
            'body': self.get_insert_body(self.data)
        }
        self.invoke_api(client, self.method_spec['op'], params)
