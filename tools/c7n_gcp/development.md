...
#### Preserving Infrastructure During Development

When actively developing and iterating on tests, you can avoid repeatedly creating/destroying infrastructure by using the `teardown` parameter:

```python
from pytest_terraform import terraform

@terraform('vertexai_endpoint', teardown=terraform.TEARDOWN_OFF)
def test_vertexai_endpoint_query(test, vertexai_endpoint):
    session_factory = test.record_flight_data('vertexai-endpoint-query')
    # ... test implementation
```

**Available teardown options:**
- `terraform.TEARDOWN_ON` - Always perform terraform cleanup, fail on error (default)
- `terraform.TEARDOWN_OFF` - Never perform terraform cleanup (useful for development)
- `terraform.TEARDOWN_IGNORE` - Always perform terraform cleanup, ignore errors (useful when tests delete resources)

**Development workflow with TEARDOWN_OFF:**

1. **First run** - Create infrastructure and record flight data:
   ```bash
   C7N_FUNCTIONAL=yes pytest tools/c7n_gcp/tests/test_vertexai.py::test_vertexai_endpoint_query -xvs
   ```

2. **Iterate on tests** - Infrastructure remains, just re-record flight data as needed:
   ```bash
   C7N_FUNCTIONAL=yes pytest tools/c7n_gcp/tests/test_vertexai.py::test_vertexai_endpoint_query -xvs
   ```

3. **Manual cleanup** when done iterating:
   ```bash
   cd tools/c7n_gcp/tests/terraform/vertexai_endpoint
   terraform destroy
   ```

4. **Before committing** - Remove `teardown=terraform.TEARDOWN_OFF` to restore default cleanup behavior

**Note:** This approach is particularly useful when:
- Developing new resources with slow infrastructure provisioning (e.g., Vertex AI endpoints)
- Iterating on test logic without changing the infrastructure
- Debugging test failures that require multiple runs
